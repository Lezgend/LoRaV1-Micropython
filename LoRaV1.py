"""
Class to interact with the L&E LoRa Module Version 1
...
class AcSIP_S76S(object):
    ...
    # Set SR Pins for GPS
    def setGPS(self):
        # SR Pins for GNSS or GPS
        pin32 = Pin(32, Pin.OUT)
        pin33 = Pin(33, Pin.OUT)

        pin32.value(not pin32.value())
        pin33.value(not pin33.value())

    def GPS(self):
        uart = UART(self.uart_id, baudrate=9600, bits=8, parity=None, stop=1, timeout=5000)
        gps = MicropyGPS()
        led = Pin(2, Pin.OUT)
